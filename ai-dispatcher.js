...
// ── OFFLINE FALLBACK ──────────────────────────────────
// Когда бэкенд недоступен — базовые ответы
function handleOffline(text) {
  const t = text.toLowerCase();
  const cities = ['тбилиси','батуми','поти','кутаиси','рустави','ереван','баку','стамбул'];
  const foundCities = cities.filter(c => t.includes(c));

  if (foundCities.length >= 2) {
    const from = cap(foundCities[0]);
    const to   = cap(foundCities[1]);
    aiState.from = from; aiState.to = to;

    // Локальный поиск
    const found = (window.allLoads || []).filter(l =>
      l.from.toLowerCase().includes(foundCities[0].slice(0,4)) &&
      l.to.toLowerCase().includes(foundCities[1].slice(0,4)) &&
      l.status !== 'taken'
    ).slice(0, 3);

    if (found.length > 0) {
      addBotMsg(`${from} → ${to}, нашёл ${found.length} груза:`);
      showFoundLoads(found.map(l => ({
        from: l.from, to: l.to, kg: l.kg,
        price: l.price, scope: l.scope, company: l.co
      })));
    } else {
      addBotMsg(`По маршруту ${from} → ${to} сейчас нет грузов. Попробуйте другой маршрут.`);
    }
  } else {
    addBotMsg('Связь с сервером прервалась. Попробуйте ещё раз через секунду.');
  }
}
